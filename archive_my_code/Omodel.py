...
#############
class SimpleQuantityClassifier(nn.Module):
    '''
    num_classes: 0, 1, 2, >=3
    '''
...
class QuantityClassifier(nn.Module):
    ...
    def __init__(self,c_in=768,num_classes = 4, pool='avg'):
        ...
        self.logits = nn.Sequential(
            ...
            nn.BatchNorm1d(c_in//4),
            nn.ReLU(),
            nn.Dropout(p=0.1),
            nn.Linear((c_in//4), num_classes),
        )
...
class QuantityClassifierV2(nn.Module):
    def __init__(self, input_dim=768, hidden_dim1=512, hidden_dim2=256, num_classes=4, pool='avg'):
        super(QuantityClassifierV2, self).__init__()
    ...
    def forward(self, emb):
        x = self.pool(emb).view(emb.size(0), -1)
        x = self.fc1(x)
        x = self.bn1(x)
        x = self.relu1(x)
        x = self.dropout1(x)
...
class QuantityClassifierV3(nn.Module):
    '''
    num_classes: 0, 1, 2, >=3
    '''
    def __init__(self,c_in=768,hidden_dim1 = 256, num_classes = 4, pool='avg'):
        super().__init__()
        self.pool = pool
        if pool == 'avg':
            self.pool = nn.AdaptiveAvgPool2d(1)
...
class BinaryClassifier(nn.Module):
    '''
    num_classes: 0, 1 meaning the bags are the same or not
    '''
    def __init__(self,c_in=768,num_classes = 2, pool='avg'):
        super().__init__()
        self.pool = pool
        if pool == 'avg':
            self.pool = nn.AdaptiveAvgPool2d(1)
