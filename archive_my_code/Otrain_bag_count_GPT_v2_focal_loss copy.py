...
BEST_MODEL_PATH = 'Oruns/GPT/BalanceCE_FirstRun07-17_15-34_E32_[1e-05-min4e-06]/checkpoint.pth.tar'
...
T_mult = 2
# Define the run name
run_name = f"Oruns/GPT/{loss}/{datetime.now().strftime('%m-%d_%H-%M')}_E{NUM_EPOCHS}_[{backbone_lr},{classifier_lr}-min{eta_min}]"
...
if loss.lower() == "focal_loss":
    criterion = lambda pred, target: binary_focal_loss_with_logits(pred, target, alpha=0.25, gamma=2.0, reduction='mean')
    print("Loss: FOCAL")
else:
...
# criterion = torch.nn.
# [ 1.50681492,  0.4247725 ,  1.09801359 ,14.00333333]
...
for epoch in range(NUM_EPOCHS):
    ...
    for images, targetTop1s, logProbTop1s, _, _, img_path in tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{NUM_EPOCHS}"):
        ...
        # targetTop1s = to_one_hot(targetTop1s, 4).to(device)  # One-hot encode targets
    ...
    compute_per_class_accuracy(train_labels, train_preds)
    ...
    compute_per_class_accuracy(val_labels, val_preds)
