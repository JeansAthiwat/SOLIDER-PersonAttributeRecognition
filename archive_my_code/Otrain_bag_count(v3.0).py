...
BEST_MODEL_PATH = 'Oruns/new_ds/1mlp_BalanceCE_FifthRun_07-17_13-07_e60_[1e-05,0.004]_lrmin5e-06/checkpoint.pth.tar'
...
T_mult = 2
# Define the run name
run_name = f"Oruns/new_ds/1mlp_BalanceCE_SixthRun_{datetime.now().strftime('%m-%d_%H-%M')}_e{NUM_EPOCHS}_[{backbone_lr},{classifier_lr}]_lrmin{eta_min}"
...
criterion = torch.nn.CrossEntropyLoss(weight=class_weights_tensor,label_smoothing=0.15).to(device)
